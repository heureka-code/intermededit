...
impl Letters {
    pub(super) fn new(mask: u32) -> Letters {
        Self(mask)
    }
}
...
mod tests {
    ...
    #[test]
    fn powers_of_two() {
        assert_eq!(2u32.pow(31), POWERS_OF_TWO[0]);
        for (i, power) in POWERS_OF_TWO.iter().rev().enumerate() {
            assert_eq!(2u32.pow(i as u32), *power);
        }
        assert_eq!(2u32.pow(0), POWERS_OF_TWO[31]);
    }
}
